...
def main(training_config, evaluation_config, directory):
    ...
    score, result = objective_function.evaluate_params(
        optimised_params, return_rewards=True
    )

    print("Detailed results with best parameters:")
    pprint(result, indent=4)
    # Save the result_to a file as a json with name evaluation_results.json
    with open(directory + "/evaluation_results.json", "w") as f:
        json.dump(result, f)
